...
- *[UPDATE 2022-08-30] Summing up the previous three points: the price cap on non-gas power plants has features of a tax on electricity production from non-gas power plants that is used to subsidize electricty production from gas power plants. But it would be a dynamic "tax" that is in each period equal to the difference between the buyers' price and the price cap in the spot market and unlike an actual tax, it would not increase the state's budget but reduce buyers' prices. If the electricity is sold via spot market, think that without the "tax" the non-gas producers would get the buyers' price, but with the "tax" they only get the price cap. If electricity poduction by a non-gas plant is triggered by an OTC contract, the same "tax" has to be paid. I know that Axel Ockenfels is floating a proposal of a tax on non-gas power production (I will add a link once the proposal is on the web), i.e. there may be many similarities (see also next point). I also need to look deeper into the relationship to the Iberian market design. If I understood correctly, they capped fuel prices for gas power plants and the fuel price subsidy was also financed by a fee.*
